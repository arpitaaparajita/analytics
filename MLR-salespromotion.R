...
fitted(fit2) # all the predicted values for fit2
omni$sales # actual values
residuals(fit2)
summary(residuals(fit2))
summary(fit2) #summary of residuals is compared, error is less so model is significant
...
(ndata2 = data.frame(price=c(60,70), promotion=c(300,400))) #variable names should be correct
...
cbind(omni,fitted(fit2)) #all the predictive data should be same
#output of residual formula and residual command is same 
cbind(omni,fitted(fit2),omni$sales-fitted(fit2),residuals(fit2))

#Assumptions
par(mfrow=c(2,2)) #to keep all 4 plots in 1 screen by multiple frames per row
plot(fit2)
par(mfrow=c(1,1)) # revert back to one by one plots
