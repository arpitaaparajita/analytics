...
summary(fit)
range(women$height)
(ndata = data.frame(height=c(58.5,60.7))) # value should be between range 58 and 72
(p=predict(fit,newdata = ndata)) #predict the weight values according to mathematical formula
cbind(ndata,p)
plot(fit)  #plot command plots 4 craphs.. see one by one nad check for following assumptions
#assumption1: homoscedasticity 
# normality of distribution, plot qq plot and if line is around points
#4th one check the points of outliers




#explained in steps in details
data(women)
women
names(women)
str(women)
